...
def analyze_consumption_patterns(df, date_columns, tesisat_col, bina_col):
    ...
    for idx, row in df.iterrows():
        ...
        # 5. ANI DÜŞÜŞ TESPİTİ - Kış aylarında ani düşüş (DÜZELTİLMİŞ - SIFIR DEĞERLERİ DAHİL)
        kis_aylari = cons_df[cons_df['season'] == 'Kış'].copy()
        
        if len(kis_aylari) >= 3:  # En az 1 tam kış sezonu
            # Kış sezonunu düzgün gruplamak için: Aralık ayını bir sonraki yıla ata
            ...
            kis_yillik = kis_aylari.groupby('kis_yili')['consumption'].mean()
            
            # DÜZELTİLMİŞ: Sıfır değerleri TUTUYORUZ (filtrelemiyoruz)
            # Böylece sıfıra düşen kış sezonlarını da görebiliriz
            yillik_ortalamalar = kis_yillik
            ...
            if len(yillik_ortalamalar) >= 2:
                ...
                for i in range(1, len(yillar)):
                    ...
                    if (onceki_tuketim >= min_onceki_kis_tuketim and 
                        ...
                        # Özel mesajlar: Sıfıra düşmüş mü?
                        if mevcut_tuketim == 0:
                            anomalies.append(f"🚨 KRİTİK: Kış tüketimi SIFIRA düştü! {onceki_yil-1}/{onceki_yil} kışı ({onceki_tuketim:.1f}) → {mevcut_yil-1}/{mevcut_yil} kışı (0), %100 düşüş")
                        elif mevcut_tuketim < 10:
                            anomalies.append(f"🚨 KRİTİK: Kış tüketimi neredeyse sıfır! {onceki_yil-1}/{onceki_yil} kışı ({onceki_tuketim:.1f}) → {mevcut_yil-1}/{mevcut_yil} kışı ({mevcut_tuketim:.1f}), %{dusus_orani:.1f} düşüş")
                        else:
                            anomalies.append(f"Ani kış düşüşü: {onceki_yil-1}/{onceki_yil} kışı ({onceki_tuketim:.1f}) → {mevcut_yil-1}/{mevcut_yil} kışı ({mevcut_tuketim:.1f}), %{dusus_orani:.1f} düşüş")
                ...
                if len(yillar) >= 2:
                    ...
                    if len(son_iki_yil) == 2:
                        ...
                        if (onceki_son >= min_onceki_kis_tuketim and 
                            ...
                            if mevcut_son == 0:
                                anomalies.append(f"🚨 SON YIL KRİTİK: Kış tüketimi SIFIRA düştü! {son_iki_yil[0]-1}/{son_iki_yil[0]} → {son_iki_yil[1]-1}/{son_iki_yil[1]}, %100 düşüş")
                            elif mevcut_son < 10:
                                anomalies.append(f"🚨 SON YIL KRİTİK: Kış tüketimi neredeyse sıfır! {son_iki_yil[0]-1}/{son_iki_yil[0]} → {son_iki_yil[1]-1}/{son_iki_yil[1]}, %{dusus_orani:.1f} düşüş")
                            else:
                                anomalies.append(f"Son yıl ani düşüş: {son_iki_yil[0]-1}/{son_iki_yil[0]} → {son_iki_yil[1]-1}/{son_iki_yil[1]}, %{dusus_orani:.1f} düşüş")
        ...
        # Ani düşüş bilgisi için ek analiz (DÜZELTİLMİŞ)
        kis_trend = "Stabil"
        if len(kis_aylari) >= 3:
            kis_aylari_trend = kis_aylari.copy()
            if 'kis_yili' not in kis_aylari_trend.columns:
                kis_aylari_trend['kis_yili'] = kis_aylari_trend.apply(
                    lambda row: row['year'] if row['month'] in [1, 2] else row['year'] + 1, 
                    axis=1
                )
            
            kis_yillik = kis_aylari_trend.groupby('kis_yili')['consumption'].mean()
            yillik_ortalamalar = kis_yillik  # DÜZELTİLMİŞ: Sıfır filtresini kaldırdık
            ...
            if len(yillik_ortalamalar) >= 2:
                ...
                # İlk yıl yüksek tüketim varsa
                if ilk_yil >= min_onceki_kis_tuketim:
                    if son_yil == 0:
                        kis_trend = "🚨 SIFIRA DÜŞTÜ"
                    elif son_yil < ilk_yil * 0.1:
                        kis_trend = "🚨 Kritik Düşüş (%90+)"
                    elif son_yil < ilk_yil * 0.5:
...
- Örn: Aralık 2023 + Ocak 2024 + Şubat 2024 = 2023/2024 kışı
""")
